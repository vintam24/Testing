	...
	class OrderReportController extends AppController{
		...
		public function index(){
			...
			foreach($orders as $key => $row){
				$arr[$key] = array($row['Order']['name'] => []);
				foreach($row['OrderDetail']as $key1 => $ordetail){
					foreach($portions as $key2 => $row2){
						if($ordetail['item_id'] == $row2['Item']['id']){
							foreach($row2['PortionDetail'] as $key3 => $row3){
								array_push($arr[$key][$row['Order']['name']], array($row3['Part']['name'] => $row3['value']));
							}
						}
					}
				}
